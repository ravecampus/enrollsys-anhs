<template>
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-calendar-plus-o"></i> SHS Grade</h1>
                <!-- <p>Set up Students</p> -->
                </div>
            </div>
            <div class="row">
                            ...
                            <data-table class="mt-2 table-bordered" :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy">
                                <tbody>
                                    <tr class="tr-shadow" v-for="(list, idx) in grades" :key="idx">
                                        ...
                                        <td>{{ list.first_quarter }}</td>
                                        <td>{{ list.second_quarter }}</td>
                                        <td>{{ list.average }}</td>
                                ...
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input class="form-control" v-model="post.first_quarter" type="number" placeholder="" @input="solveGrade()" autofocus>
                                            <span class="errors-material" v-if="errors.first_quarter">{{errors.first_quarter[0]}}</span>
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label class="control-label">2nd Quarter</label>
                                            <input class="form-control" v-model="post.second_quarter" type="number" placeholder="" @input="solveGrade()">
                                            <span class="errors-material" v-if="errors.second_quarter">{{errors.second_quarter[0]}}</span>
                                        </div>
...
export default {
    ...
    data(){
        ...
        let columns =[
            ...
            {label:'1ST Q', name:null},
            {label:'2ND Q', name:null},
    ...
    methods: {
        ...
        saveGrade(){
            if(this.post.id > 0){
                this.$axios.get('sanctum/csrf-cookie').then(response=>{
                    this.btncap = "Saving..."
                    this.btndis = true;
                    this.$axios.put('api/shs-grade/'+this.post.id, this.post).then(res=>{
                        // this.post = {};
                        this.btncap = "Save"
                        this.btndis = false;
        ...
        listOfGrade(urls='api/shs-grade'){
            this.$axios.get('sanctum/csrf-cookie').then(response => {
                this.tableData.draw ++;
